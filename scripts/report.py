...
yticks = numpy.arange(ymin, ymax + 1, 1000)
...
ax2.set_ylabel('Time needed to perform one increment [clock cycles]')
...
def plot_mutex(results_dir):
    ...
    x = [row.split(' ')[0] for row in data]
    y1 = [float(norm(results_dir, row.split(' ')[4])) / float(norm(results_dir, row.split(' ')[3])) for row in data]
    ...
    additional_yticks.append(round(y3[3]))

plot_mutex(d)

ymin, ymax = ax2.get_ylim()
ax2.set_ylim(ymin, ymax + 30)

ymin, ymax = ax2.get_ylim()
yticks = numpy.arange(ymin, ymax + 1, 50)
yticks = numpy.append(yticks, additional_yticks)
